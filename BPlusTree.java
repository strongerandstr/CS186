...
/*        LeafNode cnode;
        ...
        private  BPlusTreeIterator(LeafNode leaf){
            this.leaf = leaf;
            cursor = 0;
            size = leaf.getKeys().size();
        }

        @Override
        public boolean hasNext() {
            // TODO(proj2): implement
//            if(cnode.getRightSibling().isPresent())
//                return true;
//            if(cid+1 != cnode.getRids().size())
//                return true;
//            return false;
            if(cursor==size&&(!leaf.getRightSibling().isPresent())){
                return false;
            }
            return true;
        }
...
//            {
//                while(cid == cnode.getRids().size() && cnode.getRightSibling().isPresent())
//                {
...
//            return cnode.getRids().get(cid);
            if(hasNext()){
                ...
                return leaf.getRids().get(cursor++);
            }
