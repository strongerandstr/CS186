...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        // TODO(proj2): implement
        if(keys.contains(key)) throw new BPlusTreeException("There is already contains this key");
        // 不分裂的情况
        ...
        LeafNode leaf = new LeafNode(metadata, bufferManager, newKeys, newRids, this.rightSibling, treeContext);
        this.rightSibling = Optional.of(leaf.getPage().getPageNum());
        sync();
        return Optional.of(new Pair(newKeys.get(0), leaf.page.getPageNum()));
    }
